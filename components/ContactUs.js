...
const contactInfo = {
  address: 'No.12, V.R.R. Nagar,\nVilankurichi (Po),\nCoimbatore - 641 035.',
  phones: [
    { name: 'Vijayakumar. D', role: 'Founder', display: '+91 94430 59101', wa: '919443059101' },
    { name: 'Sri Darshan. V', role: 'Co-Founder', display: '+91 94421 55833', wa: '919442155833' }
  ],
      ...
      <div className={styles.inner}>
        ...
        <div className={`reveal-left ${styles.leftCol}`} ref={leftRef}>
          ...
          <div className={styles.details}>
            <div className={styles.detail} style={{ alignItems: 'flex-start' }}>
              <span className={styles.detailIcon} style={{ marginTop: '4px' }}>
                ...
                {phoneRevealed ? (
                  contactInfo.phones.map((p, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                      <span style={{ fontWeight: 500 }}>{p.name} <span style={{ opacity: 0.7, fontSize: '0.9em', fontWeight: 400 }}>({p.role})</span></span>
                      <span>{p.display}</span>
                    </div>
                  ))
                ) : (
